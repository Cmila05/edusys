        ...
        <div class="card">
            <div class="card-header">
                <div class="float-right">
                    <a href="" class="btn btn-info btn-sm float-right" data-placement="left" data-toggle="modal"
                        data-target="#modalMin">
                        {{ __('Crear Personal') }}
                    </a>
                    <x-adminlte-modal id="modalMin" title="Nuevo Personal" size="lg" theme="info">
                        <form method="POST" action="{{ route('personal.store') }}" role="form"
                                    ...
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                ...
                                                <a type="button" class="dropdown-item text-warning" data-toggle="modal"
                                                    data-target="#modalEdit{{ $personal->id }}"
                                                    onclick="edit({{ $personal->id }})">
                                        ...
                                        <x-adminlte-modal id="modalEdit{{ $personal->id }}" title="Editar Personal"
                                            size="lg" theme='info'>
                                            <form method="POST" action="{{ route('personal.update', $personal->id) }}"
                                                role="form" enctype="multipart/form-data">
                                                {{ method_field('PATCH') }}
                                                @csrf
                                                @include('personal.formedit')
                                            </form>
                                        </x-adminlte-modal>
                                    </td>
                                </tr>
...
@section('js')
    ...
    <script>
        ...
        // Esta función se ejecutará cuando se haga clic en el botón de ver personal
        function edit(personalId) {
            // Realizar una petición AJAX para obtener la información del personal
            $.ajax({
                url: "{{ route('personal.edit', ':id') }}".replace(':id', personalId),
                method: 'GET',
                success: function(response) {
                    // Colocar la información del personal dentro del cuerpo del modal
